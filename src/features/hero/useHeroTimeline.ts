...
export function useHeroTimeline({ motion, profile, ready, useWebGL, refs }: UseHeroTimelineOptions) {
  useLayoutEffect(() => {
    ...
    const bucketImages = story.querySelectorAll<HTMLElement>('[data-hero-bucket]');
    ...
    const ctx = gsap.context(() => {
      ...
      if (reducedMotion) {
        ...
        if (!useWebGL) {
          ...
          gsap.set(bucketImages, { autoAlpha: 0 });
      ...
      if (!useWebGL) {
        // The fallback uses the same master timeline with the five individual bucket layers.
        heroTimeline
          .to(story, { autoAlpha: 1, scale: 1.02, xPercent: 0, duration: 10 }, 10)
          .to(bucketImages, { autoAlpha: 1, scale: 1, duration: 8, stagger: 0.12, ease: 'power2.inOut' }, 10)
          ...
          .to(bucketImages, { autoAlpha: 0, scale: 0.88, duration: 10, ease: 'power2.inOut' }, 32)
